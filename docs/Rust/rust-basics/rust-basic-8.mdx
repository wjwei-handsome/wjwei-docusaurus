...
## 特征 Trait
Rust中特征Trait的概念和其他语言中的接口很相似，它定义了一些方法，如果一个类型实现了某个特征，那么它就可以调用这些方法。
...
fn mian() {
    let news = News{title: "title".to_string(),author: "wjwei".to_string(), content: "Rust棒极了!".to_string()};
    let weibo = Weibo{username: "wjwei".to_string(),content: "好像微博没Tweet好用".to_string()};
...
### 特征对象
对于之前那段[无法编译通过的代码](#函数返回值类型中使用特征约束)，可以使用特征对象来解决。

比如这样一个场景，我们想要实现一系列 UI 组件，比如按钮、文本框等，我们无法知道所有的对象类型，只知道的是：
- 每个 UI 组件都是不同的类型
- 需要一个统一的类型来处理这些对象
- 每个对象都需要实现 `Draw` 特征

如果让我用 `Python` 来实现，我会定义一个抽象顶层的类，并定义一个 `draw` 方法，然后让所有的 UI 组件继承这个类，并重写 `draw` 方法。

在 Rust 中，可以使用特征对象来实现这个功能。
